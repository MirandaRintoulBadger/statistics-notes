...
knitr::include_graphics("figs/comics/ch6.jpg")
```
...
x <- 1.663
y <- 0
plot(x, y, xlim = c(1.4, 2), ylim = c(-1, 1),
     yaxt = "n", ylab = "", xlab = "")
arrows(1.543, 0, 1.782, 0, length=0.05, angle=90, code=3,
       lwd = 2)
...
plot(z, y, type = "l", ylab = "", yaxt = "n", lwd = 2)
polygon(c(z[c(l, l:h, h)]),
        c(0, y[l:h], 0),
        col = "goldenrod1")
text(0, .16, expression(paste("0.95")), cex = 1.5)
```
...
polygon(c(z[c(l, l:h, h)]),
        c(0, y[l:h], 0),
        col = "goldenrod1")
text(0, .16, expression(paste("1 - ", alpha)), cex = 1.5)
```
...
```{r,echo=F, fig.align="center"}
knitr::include_graphics("figs/confint/ci_example.png")
```
...
labels <- c("T curve", "Standard normal (Z) curve")
...
plot(x, dt(x, 1), type = "l", lwd = 2,
     ...
     main = "")
lines(x, dnorm(x), lwd = 2, col = "black")
legend(0.8, 0.5, labels,
       col = c("darkorange", "black"), lty = c(2, 1), lwd = 2)
abline(v = 0, lty = 2)
```
...
```{r, echo = F}
x <- seq(-4.5, 4.5, by = 0.01)
labels <- c("df = 1", "df = 2", "df = 5", "Standard normal")
...
plot(x, dt(x, 1), type = "l", lwd = 2,
     ...
     col = "darkorange",
     main = "t distribution")
lines(x, dt(x, 2), lwd = 2, col = "deeppink3")
lines(x, dt(x, 5), lwd = 2, col = "darkorchid3")
lines(x, dnorm(x), lwd = 2, col = "black")
legend(1.5, 0.4, labels,
       fill = c("darkorange", "deeppink3",
               "darkorchid3", "black"))   
```
